...
impl KeyBindings {
    ...
    fn set<A: Into<ScreenActionEnum>>(&mut self, action: A, ck: KeyCombination) {
        ...
        if let Some(v) = self.default_bindings.get_mut(&action_enum) {
            v.insert(ck);
        } else {
            let mut new_set = HashSet::new();
            new_set.insert(ck);
            self.default_bindings.insert(action_enum, new_set);
        }
    }
    ...
    pub fn add<A: Into<ScreenActionEnum>>(&mut self, action: A, ck: KeyCombination) -> bool {
        let action_enum: ScreenActionEnum = action.into();
        if let Some(set) = self.default_bindings.get_mut(&action_enum) {
            return set.insert(ck);
        } else {
            self.set(action_enum.clone(), ck);
            true
        }
    }
...
fn test_deserialize_keybindings() {
    ...
    assert_eq!(conf.keybindings.get(key!(q)), Some(&ScreenActionEnum::Quit),);
    assert_eq!(
        conf.keybindings.get(key!(shift - tab)),
        Some(&ScreenActionEnum::Previous),
    );
    assert_eq!(
        conf.keybindings.get(key!(ctrl - q)),
        Some(&ScreenActionEnum::Quit),
    );
    assert_eq!(
        conf.keybindings.get(key!(cmd - e)),
        Some(&ScreenActionEnum::Quit),
    );
}
